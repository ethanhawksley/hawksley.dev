...
description: 'Everything you need to know about JSON-LD and how to implement it.'
metaDescription: 'A guide to implementing JSON-LD on your personal website for better SEO, covering WebSite, Person, BlogPosting, and more with ready-to-use code snippets.'
pubDate: 2026-06-21
modDate: 2026-09-16
---

> Update: Thanks for all the positive feedback! I've turned this post into a living document to keep it updated with current best practices. My aim here is to keep it practical and cut through the fluff you'll find on typical SEO-spam blogs.

JSON-LD, also known as JSON Linked Data, is a format for adding structured data to webpages. It can aid web crawlers in understanding the semantic structure of your site, qualifying you for rich search results, and even potentially improving your search ranking.

When I first wrote this post, it had been four months since [my first post](/blog/building-astro-blog) about building this site, with ~100 hours logged in WakaTime. Since then, the site has seen plenty of polish, including dedicated JSON-LD across every page.
...
![Google search results showing hawksley.dev, with the site's name "Ethan Hawksley" highlighted](./ethan-hawksley-sitename.png)
...
Since other nodes will often reference `WebSite`, it's generally a good idea to include it on each page.
...
```json
{
  ...
  "primaryImageOfPage": "https://hawksley.dev/og/blog/hack-club-campfire.jpg"
}
```
...
<details>
  <summary>Warning - Quite Long!</summary>
...
```json
{
  ...
  "url": "https://hawksley.dev/",
  "name": "Ethan Hawksley",
  "description": "Ethan Hawksley is a British computer science student at the University of Warwick and author of The Second Maintainer: Inside the XZ Utils Backdoor.",
  "jobTitle": "Author",
  "nationality": {
    "@type": "Country",
    "name": "United Kingdom"
...
- `name` - Clearly describes your name.
...
```json
{
  ...
  "primaryImageOfPage": "https://hawksley.dev/ethan-hawksley.jpg"
}
```

It's important to use `isPartOf` to link it to your broader `WebSite` node, to create a relationship between the two nodes. The same applies for `mainEntity`, it lets crawlers know who the page is about.

Including `dateCreated` and `dateModified` is a good freshness signal for crawlers, but if your site doesn't have them readily available, don't worry too much about it.

## BreadcrumbList

`BreadcrumbList` is widely useful and should be included on all pages that are two layers deep in your hierarchy. For example, I include it on my blog posts (/blog/POST_ID), but not on my blog index (/blog), nor on my home page (/).

A `BreadcrumbList` describes the conceptual path to a page, which does not necessarily have to match the path in the URL bar.
...
![Google search results for The Shipwrecked Hackathon by Hack Club, with the breadcrumbs highlighted](./hawksley-dev-breadcrumbs.png)
...
The `CollectionPage` node is a subtype of `WebPage`, usable for pages that primarily contain lists. For example, my /elsewhere page lists all my other profiles, and /blog page lists all my blog posts.
...
You've met most of these properties already, so they are largely self-explanatory.

## Blog

You should add the `Blog` node to your blog's index, or your home page if you don't have an index. It acts as a stepping stone between your `WebSite` and the individual blog posts you publish.
...
![Definition of publisher property, where available types include Organization and Person](./jsonld-publisher-organization-person.png)
...
```json
{
  ...
  "image": "https://hawksley.dev/og/blog/need-for-post-quantum-cryptography.jpg"
}
```
